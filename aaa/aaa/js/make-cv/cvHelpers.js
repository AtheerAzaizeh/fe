...
export async function submitCV() {
  ...
  try {
    const response = await fetch("http://localhost:3010/cvs", {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to submit CV: ${errorData.message}`);
    }
    ...
    document.getElementById('cv-error').textContent = `Failed to submit CV. Error: ${error.message}`;
  }
}
